...
export default function ServicosPage() {
  ...
  function goTo(serviceId: string, index: number) {
    setActiveSlide((prev) => ({ ...prev, [serviceId]: index }));
  }

  function changeSlide(serviceId: string, delta: number) {
    setActiveSlide((prev) => {
      const service = services.find((item) => item.id === serviceId);
      if (!service) return prev;

      const current = prev[serviceId] ?? 0;
      const next = Math.min(Math.max(current + delta, 0), service.images.length - 1);
  ...
  return (
    <main id="main-content" className="relative min-h-screen overflow-hidden bg-ink text-off">
      ...
      <section className="relative mx-auto max-w-7xl px-6 py-14 sm:py-16 lg:py-24">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">Serviços 747 Garage</h1>
            <p className="mt-3 max-w-2xl text-sm text-off/72 sm:text-base">
              Serviços técnicos para elevar acabamento, montagem e padrão de apresentação da sua Mercedes.
            </p>
          </div>
              ...
              >
                <div className="relative overflow-hidden rounded-2xl">
                  ...
                  {service.images.length > 1 && (
                    <button
                      onClick={() => changeSlide(service.id, -1)}
                ...
                <div className="mt-6 space-y-3">
                  <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-start">
                    <div>
                      <h2 className="text-xl font-semibold">
                        <Link href={`/servicos/${service.id}`} className="transition hover:text-gold">
                          {service.title}
                        </Link>
                      </h2>
                  ...
                  <p className="text-sm text-off/80">{service.description}</p>
